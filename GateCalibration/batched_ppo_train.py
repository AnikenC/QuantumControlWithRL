...
from environment import GateCalibrationEnvironment

from GateCalibrationClasses import AlternateSyncVectorEnv
...
def make_env(env_id, idx, capture_video, run_name, gamma):
    def thunk():
        env = GateCalibrationEnvironment(simple_sample=True, simple_size=2)
...
class Agent(nn.Module):
    ...
    def get_action_and_value_batched(self, x, num_steps, num_envs, action=None):
        ### When varying observations are inputted, use a for loop instead
        ...
        logprobs[:] = temp_probs.log_prob(action).sum(1)

        return actions, logprobs, 0, values
